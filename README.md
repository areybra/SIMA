...
<p align="center">
  ...
  <br/>
  <sub>Template-only (no SPA) • SQLite dev → MySQL prod • Jazzmin Admin • Mobile-first • v0.0.4 — DB URL auto-fix</sub>
</p>
...
| **DB** | SQLite (dev) ↔ MySQL/MariaDB (`PyMySQL`) via `DB_ENGINE` |
...
| `DATABASE_URL` | `mysql://root:pass@localhost:3306/sima` | **Prioritas utama**, `DB_ENGINE` diabaikan jika ada |
| `DB_ENGINE` | `sqlite` / `mysql` | Fallback jika `DATABASE_URL` kosong |
| `DB_HOST` / `DB_PORT` / `DB_NAME` / `DB_USER` / `DB_PASSWORD` | — | Lihat `.env.example` untuk split mode |
| `DB_CONN_MAX_AGE` | `60` | Koneksi persisten |

> **Auto DB di deploy:** `config/settings.py` memprioritaskan `DATABASE_URL`. Jika `DATABASE_URL` kosong, fallback ke `DB_ENGINE` (default `sqlite`).
> `.env` di-ignore git. `.env.example` sudah berisi `DATABASE_URL` MySQL aktif — `cp .env.example .env` langsung jalan untuk dev, atau kosongkan untuk `sqlite`.

---

## 🗄️ Database — SQLite / MySQL (via DATABASE_URL)

```env
# Cara 1 — DATABASE_URL (direkomendasikan, otomatis di deploy)
DATABASE_URL=mysql://root:pass@localhost:3306/sima
...
Prioritas: `DATABASE_URL` > `DB_ENGINE`. Deploy Vercel/Railway → `DATABASE_URL` MySQL otomatis terpakai. Lokal `DEBUG=True` tanpa `DATABASE_URL` → `sqlite`.
